...
@app.route('/')
def home():
    return "BOTAS ONLINE (MARKET ĮĖJIMAS + FVG TP DALYS)", 200
...
def webhook():
    try:
        ...
        # --- KIEKIS IR KONTRAKTAI (SVEIKIEJI SKAIČIAI BE PENGU KLAIDŲ) ---
        ...
        # --- ATIDARYMAS (MARKET ORDER - ĮEINAME AKIMIRKSNIU) ---
        open_params = {
            'posSide': 'SHORT',
            'openType': 1,
        ...
        order = exchange.create_order(
            symbol=symbol,
            type='market',        # Keičiame į Market, kad įeitų akimirksniu
            side='sell',
            amount=amount,
            price=None,
            params=open_params
        )
        print(f"SHORT MARKET OPENED | {symbol} | Qty={amount} | Est.Price={entry_price}")
